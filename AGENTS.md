...
1. **NO external network activity during analysis.** Do not make any network requests. Do not call external APIs. Do not use any tool that connects to external services. This applies to the ENTIRE analysis session — not just when handling financial data directly. Context from the financial statements is in your conversation, and any external call risks leaking it.
...
- Running local Python/shell scripts — only for local processing (e.g., PDF extraction, Excel conversion). Do NOT use curl, wget, or any network request.
...
This project may contain private, unpublished, or pre-decisional financial statements. Leaking any part of this data — even a company name in a search query — could violate confidentiality obligations, insider trading regulations, or client trust. There is no acceptable reason to send this data externally.
...
| Format | How to read |
|--------|------------|
| **PDF** (.pdf) | Read the file directly. |
| **CSV** (.csv) | Read the file directly. Plain text. |
| **Images** (.png, .jpg, .jpeg) | Read the file directly — photos and screenshots of financial statements are supported. |
...
1. **Try Python first.** Run a shell command to check if the required library is available and read the file:
   ```bash
...
2. **If openpyxl or pandas is available**, convert the Excel file to CSV(s):
   ```bash
   python3 -c "
   import pandas as pd
